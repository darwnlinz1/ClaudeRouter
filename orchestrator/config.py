...
MAX_WORKSTREAMS_PER_MANAGER_SLOT = int(os.environ.get("ORCH_MAX_WORKSTREAMS_PER_MANAGER_SLOT", "1"))
RATE_LIMIT_COOLDOWN_SECONDS = int(
    os.environ.get("ORCH_RATE_LIMIT_COOLDOWN_SECONDS", str(5 * 60 * 60))
)
AUTO_CONTINUE_MAX_CYCLES = int(os.environ.get("ORCH_AUTO_CONTINUE_MAX_CYCLES", "20"))

# Manager work-item capacity comes exclusively from each task's frontend-backed
# SchedulerLimits. Keep only the per-package file safety cap as a server guard.
...
HIERARCHY_MAX_SOURCE_FILES = int(os.environ.get("ORCH_HIERARCHY_MAX_SOURCE_FILES", "32"))
HIERARCHY_SOURCE_FILE_CHARS = int(os.environ.get("ORCH_HIERARCHY_SOURCE_FILE_CHARS", "24000"))
HIERARCHY_DIRECTOR_SOURCE_CHARS = int(
    os.environ.get("ORCH_HIERARCHY_DIRECTOR_SOURCE_CHARS", "80000")
)
HIERARCHY_MANAGER_SOURCE_CHARS = int(os.environ.get("ORCH_HIERARCHY_MANAGER_SOURCE_CHARS", "48000"))
TEST_TIMEOUT_SECONDS = int(os.environ.get("ORCH_TEST_TIMEOUT_SECONDS", "300"))
SANDBOX_CANCELLATION_POLL_SECONDS = float(
    os.environ.get("ORCH_SANDBOX_CANCELLATION_POLL_SECONDS", "0.05")
)
SCOPE_CLAIM_TIMEOUT_SECONDS = float(os.environ.get("ORCH_SCOPE_CLAIM_TIMEOUT_SECONDS", "900"))
