...
from .effects import MUST_BE_ABSENT, EffectState, PreparedFileEffect
...
def execute_work_item(
    *,
    ...
    worker_feedback = str(
        worker_result.tool_input.get("worker_feedback", "")
    ...
    if worker_result.tool_input.get("task_status") == "failed":
        ...
        return TicketExecutionResult(
            ...
            "",
            "not_run",
            "Manager cần điều chỉnh chỉ thị hoặc phạm vi.",
            None,
            0,
            0,
            "not_run",
            ...
            and approval_callback(
                {
                    ...
                    "target": normalized,
                    "reason": (
                        f"{risk_level} risk work contract requires human approval"
                    ),
    ...
    if not policy_decision.allowed:
        ...
        return TicketExecutionResult(
            ...
            "",
            "not_run",
            "Human approval or policy changes are required before applying this patch.",
            patch_hash,
            additions,
            deletions,
            "not_run",
    ...
    effect_fencing_token = (
        getattr(project_lease, "fencing_token", None)
    ...
    def rollback_with_receipt(reason: str) -> None:
        ...
        if effect_repository is not None and task_id:
            if (
                ...
                and effect_receipt.state is EffectState.PENDING
            ):
                reconciled_original = (
            ...
            if effect_receipt is not None:
                durable_original = effect_repository.get_effect(
                    effect_receipt.effect_id
                )
        ...
        try:
            ...
            if (
                ...
                and rollback_receipt.state is EffectState.PENDING
            ):
    ...
    try:
        with lock:
            ...
            effect_key = f"patch:{work_item_id}:{normalized}:{patch_hash}"
            if effect_repository is not None and task_id:
            ...
            if (
                effect_receipt is not None
                and effect_receipt.state
                in {EffectState.APPLIED, EffectState.RECONCILED}
            ):
                ...
                expected_applied_sha256 = (
                    prepared_patch.file_effect.after_sha256
                )
                prepared_before = (
                    None
                ...
                if effect_repository is not None and task_id:
                    try:
                        ...
                        project_workspace.discard_prepared_file(
            ...
            if syntax_status == "failed":
                ...
                sandbox_isolation = str(
                    sandbox_details.get("actual_isolation") or "none"
                )
            ...
            _emit(
                ...
                file_path=normalized,
                requested_isolation=sandbox_details.get(
                    "requested_isolation"
                ),
            ...
            feedback = str(review.tool_input.get("reviewer_feedback", ""))[
                : config.REVIEWER_FEEDBACK_MAX_CHARS
            ]
            ...
            elif on_file_approved is not None:
                on_file_approved(normalized)
            ...
            _emit(
                ...
                summary=(
                    f"Tester trả {verdict} cho {normalized}: "
                    f"{feedback or next_instructions}"
                )[:1000],
            )
            _emit(
                on_event,
        ...
        rollback_conflict = (
            exc if isinstance(exc, safety.RollbackConflictError) else None
        )
        if (
            effect_receipt is not None
            and effect_receipt.state is EffectState.PENDING
